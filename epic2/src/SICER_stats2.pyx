...
def _poisson(i, avg):
    ...
    if i < 20:
        return exp(-avg) * avg**i / _factorial(i)
    else:
        exponent = -avg + i * log(avg) - _factln(i)
        return exp(exponent)


@lru_cache(maxsize=int(1e6))
def _window_score(i, avg, bin_size):

    prob = _poisson(i, avg)
    if (i < avg):  # only want to look at enrichment
        return 0
    else:
        if prob > 0:
            return -log(prob)
        else:  #prob is too small and deemed 0 by the system
            return int(round(1000 / bin_size))
...
class Background_island_probscore_statistics:
    ...
    def __init__(self, total_tags, windowSize, gapSize, window_pvalue,
        ...
        for index in xrange(self.max_index):
            ...
            prob = _poisson(index, self.average)
        ...
        while (sf > window_pvalue):
            ...
            sf -= _poisson(self.min_tags_in_window, self.average)
        ...
        score = -log(_poisson(self.min_tags_in_window, self.average))
    ...
    def background_island_expectation(self, scaled_score):
        ...
        if scaled_score > current_max_scaled_score:
            ...
            for index in range(current_max_scaled_score + 1, scaled_score + 1):
                ...
                while (int(
                        round(index -
                              _window_score(i, self.average, self.bin_size) /
                              self.bin_size)) >= 0):
                    #while ( (index - self.window_scaled_score[i])>=0):
                    temp += _poisson(
                        i, self.average) * self.island_expectation[int(
                            round(index -
                                  _window_score(i, self.average, self.bin_size)
                                  / self.bin_size))]
